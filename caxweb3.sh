...
while true; do
  ...
  case "$CHOICE" in
    ...
    "2")
      ...
      http GET "https://cax.piccadilly.autonity.org/api/balances/" "API-Key:$(./tools/get_aut_api_key)"
      ;;
    "3")
      ...
      http GET "https://cax.piccadilly.autonity.org/api/orderbooks/$PAIR/quote" "API-Key:$(./tools/get_aut_api_key)"
      ;;
    "4")
      # Making an HTTP GET request to orderbooks without a specific pair
      http GET "https://cax.piccadilly.autonity.org/api/orderbooks/" "API-Key:$(./tools/get_aut_api_key)"
      ;;
    "5")
      ...
      http POST "https://cax.piccadilly.autonity.org/api/orders/" "API-Key:$(./tools/get_aut_api_key)" "pair=$PAIR" "side=$SIDE" "price=$PRICE" "amount=$AMOUNT"
      ;;
    "6")
      # Making an HTTP GET request to view open orders with order ID 0 and status "open"
      API_RESPONSE=$(http GET "https://cax.piccadilly.autonity.org/api/orders" "API-Key:$(./tools/get_aut_api_key)" | jq 'map(select(.status == "open"))')
    ...
    "7")
      ...
      case "$CANCEL_OPTION" in
        "1")
          ...
          SPECIFIC_ORDER_ID=$(http GET https://cax.piccadilly.autonity.org/api/orders "API-Key:$(./tools/get_aut_api_key)" | jq -r 'first(.[] | select(.status == "open") | .order_id)')

          # Check if there is an open order
          if [ -n "$SPECIFIC_ORDER_ID" ]; then
            # Making an HTTP DELETE request to cancel the specific order
            http DELETE "https://cax.piccadilly.autonity.org/api/orders/$SPECIFIC_ORDER_ID" "API-Key:$(./tools/get_aut_api_key)"
        ...
        "2")
          ...
          order_ids=$(http GET https://cax.piccadilly.autonity.org/api/orders "API-Key:$(./tools/get_aut_api_key)" | jq -r '.[] | select(.status == "open") | .order_id')

          if [ -n "$order_ids" ]; then
            # Canceling each open order
            for order_id in $order_ids; do
              http DELETE "https://cax.piccadilly.autonity.org/api/orders/$order_id" "API-Key:$(./tools/get_aut_api_key)"
    ...
    "8")
      ...
      http POST "https://cax.piccadilly.autonity.org/api/withdraws/" "API-Key:$(./tools/get_aut_api_key)" "symbol=$SYMBOL" "amount=$AMOUNT"
      ;;
    "9")
      # Making an HTTP GET request to view deposit history
      DEPOSIT_HISTORY=$(http GET "https://cax.piccadilly.autonity.org/api/deposits" "API-Key:$(./tools/get_aut_api_key)")
      ...
      WITHDRAW_HISTORY=$(http GET "https://cax.piccadilly.autonity.org/api/withdraws" "API-Key:$(./tools/get_aut_api_key)")
